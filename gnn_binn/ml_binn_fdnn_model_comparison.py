...
class FDNN(nn.Module):
    ...
    def forward(self, x):
        # pass through all hidden layers with relu and dropout
...
class ModelComparison:
    ...
    def train_fdnn_fold(self, repetition_idx, fold_idx, X_train, y_train, X_val, y_val, X_test, y_test, pos_w, device, wandb_group_name):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=binn_config.dense_initial_lr, weight_decay=1e-5)
        ...
        for epoch in range(self.binn_config.num_epochs):
            ...
            train_metrics = {
                'accuracy': accuracy_score(train_labels, train_preds_binary),
                'precision': precision_score(train_labels, train_preds_binary, zero_division=0),
                'recall': recall_score(train_labels, train_preds_binary, zero_division=0),
                'f1': f1_score(train_labels, train_preds_binary, zero_division=0),
                'auc': roc_auc_score(train_labels, train_preds),
                'auprc': average_precision_score(train_labels, train_preds)
            }
    ...
    def run_comparison(self):
        ...
        # gnn-binn full data loading (done once) 
        ...
        gnn_binn_df = pd.DataFrame(all_metrics['GNN-BINN'])
        binn_df = pd.DataFrame(all_metrics['BINN'])
        gnn_binn_df.to_csv(os.path.join(self.gnn_binn_config.plot_save_path, 'gnn_binn_results.csv'), index=False)
        binn_df.to_csv(os.path.join(self.gnn_binn_config.plot_save_path, 'binn_results.csv'), index=False)
